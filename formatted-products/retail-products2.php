...
<body>
    ...
    <section>
        ...
        <?php
        try {
            // Build SQL query to select all product fields, sorted by product name in descending order
            $sql = "SELECT product_image, product_name, product_description, product_price, product_status, product_inStock 
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                // Display product image from product image folder
                echo '<div class="productImage">';
                echo '<img src="productImages/' . htmlspecialchars($row["product_image"]) . '"
                ...
                // Display product price formatted to 2 decimals
                echo '<p class="productPrice">$' . number_format($row["product_price"], 2) . '</p>';

                // Display product status if there are status updates
                if (!empty($row["product_status"])) {
                    echo '<p class="productStatus">' . htmlspecialchars($row["product_status"]) . '</p>';
                }

                // Display inventory with conditional formatting
                // *out of stock if 0
                // *Low inventory if less than 10
                if ($row["product_inStock"] == 0) {
                echo '<p class="productInventory productLowInventory">Out of Stock</p>';
                } elseif ($row["product_inStock"] < 10) {
                    echo '<p class="productInventory productLowInventory">Low Inventory: ' . intval($row["product_inStock"]) . '</p>';
                } else {
                    echo '<p class="productInventory">In Stock: ' . intval($row["product_inStock"]) . '</p>';
                }

                // close product block container
                echo '</div>';
            }

        // Catch and display errors if query fails
